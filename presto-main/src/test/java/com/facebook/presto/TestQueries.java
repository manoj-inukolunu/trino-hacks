...
public class TestQueries {
    ...
    public void testReorderOutputsOfGroupByAggregation()
            throws Exception {
        assertQuery("SELECT orderstatus, a, custkey, b FROM (SELECT custkey, orderstatus, -COUNT(*) a," +
                " MAX(orderkey) b FROM ORDERS WHERE orderkey = 1 GROUP BY custkey, orderstatus) T");
    }

    @Test
    public void testGroupAggregationOverNestedGroupByAggregation()
            throws Exception {
        assertQuery("SELECT sum(custkey), max(orderstatus)," +
                " min(c) FROM (SELECT orderstatus, custkey, COUNT(*) c FROM ORDERS GROUP BY orderstatus, custkey) T");
    }
   ...
   /* @Test
    ...
    @Test
    public void testManojJoin() throws Exception {
        assertQuery("SELECT * FROM lineitem JOIN orders ON lineitem.orderkey = orders.orderkey");

        Operator operator = plan("SELECT * FROM lineitem JOIN orders ON lineitem.orderkey = orders.orderkey");
    ...
    private Operator plan(String sql) {
        ...
        SubPlan subplan = new DistributedLogicalPlanner(metadata).createSubplans(plan,
                analysis.getSymbolAllocator(), true);
